...
router.post('/rooms', async (req, res) => {
  try {
    ...
    await room.save();
    await invalidateRoomCache(room._id.toString());
...
router.get('/rooms/:roomId', async (req, res) => {
  try {
    ...
    const room = await cacheService.memory.remember(
      `room-details:${roomId}:viewer:${userId}`,
      20000,
      async () => Room.findById(roomId)
        .populate('creator', 'username avatar')
    ...
    if (room.type === 'private' && !room.members.some((member) => member.user?._id?.toString?.() === userId.toString() || member.user?.toString?.() === userId.toString())) {
...
router.post('/rooms/:roomId/messages/:messageId/consume-view-once', async (req, res) => {
  try {
...
router.get('/rooms/:roomId/stats', async (req, res) => {
  try {
    ...
    if (!room.isAdmin(userId) && !arrayIncludesId(room.moderators, userId)) {
      return res.status(403).json({ message: 'Access denied' });
    }
